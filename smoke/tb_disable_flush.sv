...
module tb_disable_flush;
  ...
  initial begin
    ...
    repeat (3) @(posedge clk);
    if (link_active)
      $fatal(1, "link stayed active after administrative disable");
    if (link_fault || replay_miss || retry_exhausted)
      $fatal(1, "clean administrative disable created a fault");

    enable = 1;
    wait (link_active);
    send_payload(256'h2222);
    wait (tx_count == 2);
    if (second_seq != 8'd0)
      $fatal(1, "disable/re-enable did not reset TX sequence epoch");

    repeat (5) @(posedge clk);
    if (tx_count != 2)
      $fatal(1, "stale outstanding flit replayed after administrative flush");

    $display("DISABLE/FLUSH SMOKE PASS seqs=%0d,%0d", first_seq, second_seq);
    #20 $finish;
  end
